...
const ChatWithPersona: React.FC = () => {
  ...
  const [step, setStep] = useState<'audience' | 'segments' | 'chat'>('audience');
  ...
  useEffect(() => {
    ...
    fetchPersonaIds();
  }, [step, audienceData.selectedSegments, audienceData.personaFilters, audienceData.audienceName]);
  ...
  const handleSegmentsNext = (segments: number[], filters: Record<number, any>) => {
  ...
  if (step === 'segments' && selectedAudienceId) {
    ...
    return (
      <>
        <Header />
        <br />
        {name.includes('grove') || name.includes('nrc') ? <SegmentsSelectorGrove {...commonProps} /> :
         name.includes('yoga') ? <SegmentsSelectorYoga {...commonProps} /> :
         name.includes('kettle & fire') ? <SegmentsSelectorKettleAndFire {...commonProps} /> :
         name.includes('lovevery') ? <SegmentsSelectorLoverery {...commonProps} /> :
         name.includes('core') ? <SegmentsSelectorCoreStack {...commonProps} /> :
         name.includes('solidigm') ? <SegmentsSelectorSolidigm {...commonProps} /> :
    ...
    if (loadingPersonas) {
      return (
        <div className="flex flex-col items-center justify-center min-h-[60vh] text-gray-500">
          ...
          <div className="text-lg font-medium">Loading personas for chat...</div>
        </div>
      );
    }
    if (chatPersonaIds.length === 0) {
      return <div>No personas found for the selected segments and filters.</div>;
    }
    return (
      <>
        <Header />
        <div className="flex flex-col min-h-[80vh] items-center justify-center bg-gray-50 py-10 px-2">
          <div className="w-full max-w-6xl mx-auto mt-8">
            <div className="mb-6 flex items-center justify-between">
              <h1 className="text-2xl font-semibold text-gray-800">Chat with Personas</h1>
              <button
                onClick={() => setStep('segments')}
                className="px-4 py-2 rounded-lg bg-white border border-gray-200 text-gray-700 hover:bg-gray-100 shadow-sm"
              >
                Back to segment selection
              </button>
            </div>
            <ChatUI personaIds={chatPersonaIds} />
          </div>
        </div>
      </>
    );
