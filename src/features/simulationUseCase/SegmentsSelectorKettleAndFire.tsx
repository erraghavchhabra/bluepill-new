...
const SegmentsSelectorKettleAndFire: React.FC<SegmentsSelectorProps> = ({
  ...
  return (
    <StepContainer
      title="Select Audience Segments (Kettle & Fire)"
                ...
                >
                  ...
                  {!selectedSegments.includes(segment.id) && (
                    <div className="space-y-3 cursor-pointer flex-grow" onClick={() => toggleSegment(segment.id)}>
                      ...
                      {/* Pets & Children Tags */}
                      <div className="flex flex-wrap gap-2">
                        {[...PETS.slice(0, 1), ...CHILDREN.slice(0, 1)].map((item, i) => (
                            ...
                            {SubSegments[segment.name].map((subSegment, i) => (
                              <div key={i} className="relative inline-flex items-center">
                                <input
                                  ...
                                  checked={isFilterSelected(segment.id, 'subSegments', subSegment)}
                                  className="peer absolute opacity-0 w-0 h-0 cursor-pointer"
                                  onChange={e => toggleFilter(segment.id, 'subSegments', subSegment, e.target.checked)}
                                />
                                <label
                          ...
                          {PETS.map((pet, i) => (
                            <div key={i} className="relative inline-flex items-center">
                              <input
                                type="checkbox"
                                id={`pet-${segment.id}-${i}`}
                                checked={isFilterSelected(segment.id, 'pets', pet)}
                                className="peer absolute opacity-0 w-0 h-0 cursor-pointer"
                                onChange={e => toggleFilter(segment.id, 'pets', pet, e.target.checked)}
                              />
                              <label
                          ...
                          <div className="bg-indigo-50/50 p-5 rounded-lg border border-indigo-100">
                            ...
                            <div className="space-y-3">
                              ...
                              {selectedPersona.data?.pets && (
                                <div className="flex items-start">
                                  <span className="text-sm font-medium text-gray-600 w-28">Pets:</span>
                                  <span className="text-sm text-gray-800">{selectedPersona.data.pets}</span>
                                </div>
                              )}
                        ...
                        <div className="space-y-6 mt-4">
                          ...
                          <div className="mt-6">
                            ...
                            {selectedPersona.data && renderDynamicPersonaData(selectedPersona.data, [
                              'age_group', 'household_income', 'geo_location', 'pets', 'children'
                            ])}
