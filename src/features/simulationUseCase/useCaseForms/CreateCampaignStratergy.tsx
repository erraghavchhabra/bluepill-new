...
import {
  CampaignBrandPositionIcon,
  CampaignBudgetIcon,
  CampaignProductIcon,
  CampaignTeamIcon,
  CampaignTimelineIcon,
  CampaignWebsiteIcon,
  RightWhiteArrow,
...
const CreateCampaignStratergy: React.FC<InsightsFormProps> = ({
  ...
  const [productWebsite, setProductWebsite] = useState(
    initialFormData.productWebsite || ""
  );
  ...
  useEffect(() => {
    ...
    brandPosition,
    timeline,
  ]); // Remove onFormDataChange from dependencies
  ...
  return (
    <div className="w-full bg-gray_light rounded-tl-[30px] p-[30px] relative pb-16">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 items-center justify-between gap-5 mt-[30px]">
        ...
        <CustomInput
          id="Product"
          value={product}
          onChange={(e: any) => handleFieldChange(setProduct, e.target.value)}
          placeholder="Product*"
          icon={<CampaignProductIcon />}
        />
        <CustomInput
          id="productWebsite"
          value={productWebsite}
          onChange={(e: any) =>
            handleFieldChange(setProductWebsite, e.target.value)
          }
          placeholder="Product website*"
          icon={<CampaignWebsiteIcon />}
        />
        <CustomInput
          id="budget"
          value={budget}
          onChange={(e: any) => handleFieldChange(setBudget, e.target.value)}
          placeholder="Budget"
        ...
        <CustomSelect
          id="team"
          value={team}
          onChange={(e: any) => handleFieldChange(setTeam, e)}
          placeholder="Select team structure"
          icon={<CampaignTeamIcon />}
          options={[
        ...
        <CustomSelect
          id="brandPosition"
          value={brandPosition}
          onChange={(e: any) => handleFieldChange(setBrandPosition, e)}
          placeholder="Select brand position"
          icon={<CampaignBrandPositionIcon />}
          options={[
        ...
        <CustomInput
          id="timeline"
          value={timeline}
          onChange={(e: any) => handleFieldChange(setTimeline, e.target.value)}
          placeholder="Timeline"
          icon={<CampaignTimelineIcon />}
        />
      </div>
