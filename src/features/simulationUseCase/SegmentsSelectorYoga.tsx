...
const SegmentsSelectorYoga: React.FC<SegmentsSelectorProps> = ({
  ...
  const hasAnyFilters = (segmentId: number): boolean => {
    ...
    const filters = personaFilters[segmentId];
    return filters.ageGroups?.length > 0 || filters.geoLocation?.length > 0;
  };
  ...
  const getFilterCount = (segmentId: number): number => {
    ...
    const filters = personaFilters[segmentId];
    return (
      (filters.ageGroups?.length || 0) + (filters.geoLocation?.length || 0)
    );
  };
  ...
  const renderDynamicPersonaData = (
    data: any,
    excludeKeys: string[] = []
  ): JSX.Element | null => {
      ...
      <div className="flex items-center gap-1 justify-between">
        <div>
          <h3 className="text-[28px] font-semibold text-black mb-3">
            Select Audience Segments (LOVEVERY)
          </h3>
...
export default SegmentsSelectorYoga;
